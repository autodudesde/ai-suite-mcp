...
class GetFlexFormSchemaTool extends AbstractDataTool
{
    protected ?string $requiredScope = null;

    public function getName(): string
    {
        return 'getFlexFormSchema';
    }

    public function getDescription(): string
    {
        return 'Resolve the inner schema of a FlexForm field (default tt_content.pi_flexform): '
            .'its sheets, fields, types and select options. The data structure can depend on the '
            .'record type (CType / list_type), so pass recordUid for an existing element, or a type '
            .'hint when there is no record yet. Use getRecordSchema for the regular columns; use this '
            .'tool only to look inside a `flex` field. Respects user permissions.';
    }
    ...
    protected function doExecute(array $params): CallToolResult
    {
        ...
        if (!is_array($fieldTca) || 'flex' !== (string) ($fieldTca['config']['type'] ?? '')) {
            return $this->textResult(sprintf(
                'Field `%s.%s` is not a FlexForm (`flex`) field. Use getRecordSchema for regular columns.',
                $table,
                $field,
            ));
        }
        ...
        try {
            ...
            $this->logger->warning('GetFlexFormSchemaTool: could not resolve data structure', [
                'table' => $table,
                'field' => $field,
                'recordUid' => $recordUid,
                'type' => $typeHint,
    ...
    private function writeHint(string $field, ?string $sheetKey, ?string $fieldKey): string
    {
        ...
        $hint = "\n**How to write a value** — pass this as the `".$field.'` field value to writeRecords '
            ."(only include the fields you change):\n"
